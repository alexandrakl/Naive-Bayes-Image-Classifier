...
public:
    ...
    /**
     * @brief get hidden obstacle locations
     * @return hidden obstacle locations
     */
    std::vector<Point2D> getHiddenObstacleLocations() {return this->hiddenObstacleLocations;}
    /**
    ...
    int getCurrentNumWalls() {
        std::vector<Point2D> locs = getLocalObstacleLocations(this->robot->getPosition());
    ...
    bool step(RobotAction a)
    {
        ...
        bool didMove = true;
        if (this->isOccupied(newX, newY) || newX>=this->SX || newY>=this->SY || newX<0 || newY<0 ||
            AREA[newX][newY] == OBSTACLE || AREA[newX][newY] == HIDDEN_OBSTACLE) {didMove = false;}
        if (didMove) {
